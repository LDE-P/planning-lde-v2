...
## Process / outillage (session Prise en main ESM — Formation J2) — 2026-06-09

- [ ] **Documenter dans `CLAUDE.md` racine : Claude Code ne peut pas réaliser de captures d'écran via CDP/`localhost`** (corollaire de la limite `localhost` déjà notée). Router d'emblée les captures vers LDE (manuel) ou une session navigateur native — éviter l'aller-retour « analyse de faisabilité → bascule ».
- [ ] **Convention « déplacer un fichier référencé dans `data.json` »** : tout déplacement/renommage doit s'accompagner (a) de la MAJ des chemins `data.json`, (b) d'un grep des **liens relatifs internes** (images `](captures/…)`, renvois inter-docs). Idéalement un petit script `move-doc.py` qui fait les deux. *(Déclencheur : la session deck a laissé 2 réfs `data.json` cassées — CHARTE-v3 inexistant, RELAIS-REFONTE absent.)*
- [ ] **Coordination multi-sessions sur fichiers partagés** (`data.json`, `notes-todo.md`, `livrables/TRANSMISSION.md`) : convention « qui édite quoi » ou verrou léger — plusieurs divergences/doublons réconciliés cette session (mémo `livrables/` périmé en double, compteur de constats périmé).
- [ ] **Généraliser le pattern « 1 `.md` par bug »** (`retours-devs/BUG-NNN-*.md` + index) pour tous les retours produits aux devs — repro / cause `file:ligne` / correctif / test.

## Process / outillage (session Banc de tests — échantillon sites capturés) — 2026-06-11

- [x] **`data.json` édité via outil fichier = risque d'écrasement concurrent — INCIDENT.** Un commit Cowork a écrasé 1 projet + 12 SP d'une session parallèle (copie périmée servie par l'outil fichier vs version réécrite par le serveur). Restauré. → **Acté `CLAUDE.md` racine + `planning-lde-v2/CLAUDE.md`** : éditer `data.json` (et tout JSON servi par un serveur) **en bash, passe atomique**, avec contrôle anti-régression du nombre de projets/SP avant push. Concrétise le risque « coordination multi-sessions sur fichiers partagés » noté le 2026-06-09.
